...
func TestLedgerReplicationResponse_Pretty_okWithBlocks(t *testing.T) {
	uc1, err := (&bfttypes.UnicityCertificate{Version: 1, InputRecord: &bfttypes.InputRecord{Version: 1, RoundNumber: 1}}).MarshalCBOR()
	require.NoError(t, err)
	uc2, err := (&bfttypes.UnicityCertificate{Version: 1, InputRecord: &bfttypes.InputRecord{Version: 1, RoundNumber: 2}}).MarshalCBOR()
	require.NoError(t, err)
	r := &LedgerReplicationResponse{
		Status:  Ok,
		Message: "",
		Blocks: []*bfttypes.Block{
			{
...
func TestLedgerReplicationResponseValidation(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "ValidResponse",
			response: &LedgerReplicationResponse{Status: Ok, Blocks: []*bfttypes.Block{}},
			wantErr:  nil,
		},
		{
			name:     "NilResponse",
		...
		},
		{
			...
			response: &LedgerReplicationResponse{Status: Ok, Message: "message", Blocks: []*bfttypes.Block{}},
			wantErr:  nil,
		},
	}
