...
func TestNewPeer_InvalidPrivateKey(t *testing.T) {
	...
	_, err := NewPeer(context.Background(), conf, testlogger.New(t))
...
func TestNewPeer_LoadsKeyPairCorrectly(t *testing.T) {
	...
	peer, err := NewPeer(context.Background(), conf, testlogger.New(t))
	require.NoError(t, err)
...
func TestBootstrapNodes(t *testing.T) {
	...
	bootstrapNode, err := NewPeer(ctx, bootStrapPeerConf, log)
	require.NoError(t, err)
	...
	peer2, err := NewPeer(ctx, peerConf2, log)
	require.NoError(t, err)
...
func TestBootstrap_OneBootStrapConnectionFails_StillOK(t *testing.T) {
	...
	require.NoError(t, err)
	bootstrapNode2, err := NewPeer(ctx, bootStrapPeer2Conf, log)
	require.NoError(t, err)
	// set bootstrap info
...
func TestBootstrapConnect_OnlySelf(t *testing.T) {
	...
	testPeer, err := NewPeer(ctx, peerConf, log)
	require.NoError(t, err)
...
func TestProvidesAndDiscoverNodes(t *testing.T) {
	...
	require.NoError(t, err)
	peer3, err := NewPeer(ctx, peerConf3, log)
	require.NoError(t, err)
...
func TestAnnounceAddrs(t *testing.T) {
	...
	peer1, err := NewPeer(ctx, conf, testlogger.New(t))
	require.NoError(t, err)
...
func createBootstrappedPeer(t *testing.T, bootstrapPeers []peer.AddrInfo) *Peer {
	...
	p, err := NewPeer(context.Background(), peerConf, testlogger.New(t))
	require.NoError(t, err)
